...
df_raw_ <- as.data.frame(fread(paste0(
  pheno_dir_path,
  "phenotype_raw_data_no_outliers.csv"
)))[, c("Genotype", "Management", "Envir", trait_)]
table(df_raw_$Envir)
length(unique(df_raw_$Envir))

df_adj <- as.data.frame(fread(paste0(
  pheno_dir_path,
...
df_raw <- df_raw_[df_raw_$Management == management_type_, ]
df_adj <- df_adj[df_adj$Management == management_type_, ]

# show number of environments for management type
print(paste0(
...
print(paste0(
  "Number of environments for ", trait_,
  " raw data associated to management type ",
  management_type_, ": ",
  length(unique(df_raw$Envir))
))

# show number of environments for management type after drop_na
df_raw_ <- df_raw_ %>% drop_na(all_of(trait_))
df_raw <- df_raw %>% drop_na(all_of(trait_))

# show number of environments for management type
print(paste0(
  "Number of environments for ", trait_,
  " raw data after drop_na : ",
  length(unique(df_raw_$Envir))
))
print(paste0(
  "Number of environments for ", trait_,
  " raw data associated to management type ",
  management_type_, " after drop_na : ",
  length(unique(df_raw$Envir))
))
...
df_raw_h2_var_comp[, 1:2]
df_adj_h2_var_comp[, 1:2]
