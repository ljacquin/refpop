...
trait_ <- c("Trunk_increment")[trait_num]
...
# refpop_1_spat_hetero_correct_per_env_trait_and_h2_estim.R) and for which trait
# has available data
...
if (file.exists(paste0(
  ...
  wiser_obj <- estimate_wiser_phenotype(omic_df, raw_pheno_df, trait_,
    ...
    alpha_ = opt_alpha_,
    nrow_approx_lim_raw_dataset_zca_cor = 30e3,
    nrow_approx_lim_raw_dataset_pca_cor = 30e3
  )
  end_time_ <- Sys.time()
...
fwrite(df_pa_,
  file = paste0(
    ...
    trait_, "_", kernel_, "_kernel_", k_folds_, "_folds_CV_30e3.csv"
  ), row.names = T
)
print(df_pa_)
