...
vect_country_year_or_manag_type_as_label <- c("country", "year", "management_type")
...
# color function for countries, year and management type
...
# color function for management types
color_func_manag_type <- colorRampPalette(c(
  "red", "blue"
))
...
file_ <- files_names_spats_adj_pheno
...
for (file_ in files_names_spats_adj_pheno) {
  ...
  try({
    ...
    if (perform_umap_) {
      ...
      for (use_country_year_or_manag_type_as_label in
        vect_country_year_or_manag_type_as_label) {
        # umap plots
        ...
        if (identical(use_country_year_or_manag_type_as_label, "country")) {
        ...
        } else if (identical(use_country_year_or_manag_type_as_label, "year")) {
          color_palette <- color_func_year(length(unique(df_$year)))
          vect_labels <- df_$year
          label_title_ <- "Year"
        } else {
          color_palette <- color_func_manag_type(length(unique(df_$management_type)))
          vect_labels <- df_$management_type
          label_title_ <- "Management type"
        }
        ...
        fig_x_y <- plot_ly(
          ...
          layout(
            plot_bgcolor = "#e5ecf6",
            title = paste0("UMAP 2D plot for REFPOP G x E x M associated to ", trait_),
    ...
    if (perform_pca_) {
      ...
      for (use_country_year_or_manag_type_as_label in
        ...
        fig_x_y <- plot_ly(
          ...
          layout(
            plot_bgcolor = "#e5ecf6",
            title = paste0("PCA 2D plot for REFPOP G x E x M associated to ", trait_),
